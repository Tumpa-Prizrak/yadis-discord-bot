...
class EventsCog(Cog):
    ...
    async def _create_temp_voice_channel(self, guild: Guild, member: Member) -> VoiceChannel:
        channel: VoiceChannel = await guild.guild.create_voice_channel(f"{member.display_name}'s channel")
